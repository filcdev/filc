...
import { navigatorStairSchema } from '@/utils/form-schemas';
import {
  asNavigatorFormValidator,
  type StairDialogProps,
  selectPlaceholder,
} from './types';

const initialState = (record: NavigatorStair | null) => ({
...
type StairFormValues = ReturnType<typeof initialState>;

export function StairDialog({ record, open, onOpenChange }: StairDialogProps) {
  const { t } = useTranslation();
  const buildingsQuery = useBuildings();
  const upsert = useUpsertStair({ onSaved: () => onOpenChange(false) });
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: asNavigatorFormValidator<StairFormValues>(navigatorStairSchema),
    },
  });
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {record
              ? t('navigator.stairs.editTitle')
              : t('navigator.stairs.createTitle')}
          </DialogTitle>
        </DialogHeader>
        <form
          className="space-y-4"
