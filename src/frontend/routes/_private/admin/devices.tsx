...
import { DeviceDialog } from '~/frontend/components/doorlock/device-dialog';
import {
  Alert,
  AlertDescription,
  AlertTitle,
...
function DevicesPage() {
  ...
  const upsertMutation = useMutation({
    mutationFn: ({ id, payload }: { id?: string; payload: DevicePayload }) => {
      ...
      return parseResponse(apiClient.doorlock.devices.$post({ json: payload }));
    },
    onError: (error: Error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) =>
      parseResponse(
        apiClient.doorlock.devices[':id'].$delete({ param: { id } })
      ),
    onError: (error: Error) => {
