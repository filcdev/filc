...
import { CardDialog } from '~/frontend/components/doorlock/card-dialog';
import {
  Alert,
  AlertDescription,
  AlertTitle,
...
import { confirmDestructiveAction } from '~/frontend/utils/confirm';
...
type UsersResponse = InferResponseType<
  typeof apiClient.doorlock.cards.users.$get
>;
...
function CardsPage() {
  ...
  const usersQuery = useQuery({
    ...
    queryFn: async (): Promise<DoorlockUser[]> => {
      const res = await parseResponse(apiClient.doorlock.cards.users.$get());
    ...
    mutationFn: ({ id, payload }: { id?: string; payload: CardPayload }) => {
      if (id) {
        return parseResponse(
          apiClient.doorlock.cards[':id'].$put(
            { param: { id } },
            {
      ...
      return parseResponse(apiClient.doorlock.cards.$post({ json: payload }));
    },
    onError: (error: Error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) =>
      parseResponse(apiClient.doorlock.cards[':id'].$delete({ param: { id } })),
    onError: (error: Error) => {
